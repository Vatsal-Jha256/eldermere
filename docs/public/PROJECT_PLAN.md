...
8. Stage 7: story expansion. Use the Arthurian lore foundation to flesh out original main-story arcs, side quests, factions, companions, relics, and procedural variants while keeping modern adaptation material out of the source base. Initial validated Arthurian main and side arc data now exists in `content-packs/arthurian-core`; the server loads story arcs at startup and supports `story`, `story start`, `story status`, and `story next` with persisted progress and outcome tags. Deeper branching rules from tags, factions, and room state remain future work.
...
12. After the lore source base exists, expand main story and side arcs from that source base. Initial validated Arthurian main and side arc data is in `content-packs/arthurian-core/story_arcs.json`; runtime story browsing and persisted start/status/advance progress are available through the `story` command family, and deeper branching rules remain.
13. Add dynamic atmospheric background generation/selection for rooms and modded content. Initial room atmosphere metadata and generated CSS renderer are in place; future generated bitmap backgrounds can reuse the same metadata.
